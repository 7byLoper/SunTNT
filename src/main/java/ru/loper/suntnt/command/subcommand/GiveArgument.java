...
public class GiveArgument implements BuildableCommand {
    ...
    public void handle(@NotNull CommandSender sender, String[] args) {
        if (args.length < 2) {
            TextFormatter.send(sender, configManager.getGiveUsageMessage());
            return;
        }

        CustomTNT customTNT = tntManager.getCustomTNT(args[1]);
        if (customTNT == null) {
            TextFormatter.send(sender, configManager.getInvalidTntMessage());
            return;
        }
        ...
        if (amount <= 0) {
            TextFormatter.send(sender, configManager.getGiveInvalidAmountMessage());
            return;
        }
